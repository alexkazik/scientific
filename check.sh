...
cargo $TOOLCHAIN build --release --no-default-features --features std --tests
cargo $TOOLCHAIN build --release --no-default-features
...
cargo $TOOLCHAIN clippy --release --no-default-features --features std --tests -- -D warnings
...
# update readme
( cd scientific && cargo rdme --force )

# create docs
if test "$TOOLCHAIN" = "+nightly"
then
  RUSTDOCFLAGS="--cfg docsrs" cargo +nightly doc -p scientific --all-features
else
...
cargo $TOOLCHAIN test --locked --release --no-default-features --features debug --lib --bins --tests -- --include-ignored
