use crate::types::limited::{Exponent, Length, Limited, Unchecked};
...
pub(crate) struct Sci {
  ...
  pub(crate) len: Length,        // must be 0 for value 0, greater than 0 otherwise
...
impl Sci {
  ...
  pub(crate) const ZERO: Sci = Sci {
    ...
    len: Length::ZERO,        // required for is_zero() to work
  ...
  pub(crate) fn assign_zero(&mut self) {
    self.len = Length::ZERO; // required for is_zero() to work
  ...
  pub(crate) const fn nz_unchecked_static_new(
    ...
    Sci {
      ...
      len: Length::from_usize(mantissa.len()),
      exponent,
      owner: Owner::None,
    }
  }
  ...
  #[inline]
  pub(crate) fn exponent0(&self) -> Limited<2> {
    self.exponent + self.len
  }

  #[inline]
  pub(crate) fn exponent1(&self) -> Limited<2> {
    // Safety: len is >= 0 and thus subtracting 1 will always fit
    self.exponent + self.len - Unchecked(1)
  }
  ...
  pub(crate) fn precision_len(&self, precision: Precision) -> isize {
    match precision {
      ...
      Precision::Decimals(decimals) => self.exponent0().saturating_add(decimals),
    }
  }
}
