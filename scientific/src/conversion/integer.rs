        ...
        if value.inner.is_zero() {
          ...
          match value.inner.compare::<false>(&$const) {
            ...
            Ordering::Less => {
              ...
              result *= $ty::from(10_i8).pow(value.inner.exponent.to_isize() as u32);
    ...
    impl TryFrom<&Scientific> for $ty {
      ...
      fn try_from(value: &Scientific) -> Result<Self, Self::Error> {
        if value.inner.is_zero() {
          ...
          result *= $ty::from(10_u8).pow(value.inner.exponent.to_isize() as u32);
...
mod c_use {
  ...
  pub(super) use crate::types::limited::{Exponent, Length, ToIsize};
...
mod c_i8 {
  ...
  conversion_signed!(i8, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_u8 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_unsigned!(u8, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_i16 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_signed!(i16, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_u16 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_unsigned!(u16, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_i32 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_signed!(i32, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_u32 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_unsigned!(u32, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_i64 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_signed!(i64, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_u64 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_unsigned!(u64, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_i128 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_signed!(i128, Length::from_usize(DIGITS.len()), SCI);
}
...
mod c_u128 {
  ...
  const SCI: Sci = Sci::nz_unchecked_static_new(Sign::NEGATIVE, &DIGITS, Exponent::ZERO);
  conversion_unsigned!(u128, Length::from_usize(DIGITS.len()), SCI);
}
